<template>
  <div class="codeless-header-avatar-ctn">
    ...
    <el-dialog
      ...
      class="dialog-small adaptDialog"
      :append-to-body="true"
    >
      <codeless-dialog-ctn
        title="重置用户密码"
  ...
  methods: {
    ...
    resetPassword() {
      ...
      self.$refs.user.validate((valid) => {
        if (valid) {
          self.$store
            .dispatch("user/resetPassword", self.formData)
            .then((res) => {
    ...
    logout() {
      ...
      // });
        ...
        let _this = this;
        _this.openCustomDialog({
        title:`提示`,
        path:'deleteConfirm',
        width:"300px",
        component:{
          template:`<div>确认退出吗?</div>`,
        },
        confirmCallback(){
          _this.$store
          .dispatch("user/logout", { ctoken: getToken() })
