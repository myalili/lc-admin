<template>
  <el-dialog
    ...
    :title="type === 'addParent' ? '新增权限' : type === 'addChild'? '新增子权限' : '编辑权限'"
    :visible.sync="authDialogVisible"
    @close="closeAuthDialog"
     custom-class="no-header-and-footer-dailog auth-user-dialog-reset auth-user-dialog-top"
  >
    <codeless-Dialog-Ctn
      :title="type === 'addParent' ? '新增权限' : type === 'addChild'? '新增子权限' : '编辑权限'"
      @confirm="submitSave"
      @cancel="authDialogVisible = false"
      @close="authDialogVisible = false"
    >
    ...
    <!-- <div slot="footer" class="common-bottom-btn-ctn bottom-btn-ctn">
      <el-button class="common-cancel-btn cancel-btn" @click="closeAuthDialog"
        >取消</el-button
      >
      <el-button
...
export default {
  ...
  methods: {
    showAuthDialog(type, rowInfo) {
      console.log(type,rowInfo);
...
.auth-user-dialog-top {
  margin-top: calc(50vh - 107px) !important;
}
</style>
