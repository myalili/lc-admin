...
Uae.component('edge', {
  ...
  computed: {
    ...
    startPoint() {
      ...
      if (sBounds) {
        ...
        const y = sBounds.y + sBounds.height / 2;
        return {
          x,
          y
        };
    ...
    endPoint() {
      ...
      if (eBounds) {
        ...
        const y = eBounds.y + eBounds.height / 2;
        return {
          x,
          y
        };
    ...
    path() {
      return this.originPath.map(item => ({
        x: item.x - this.bounds.x,
        y: item.y - this.bounds.y
      }));
    },
    originTextBounds() {
      let x, y;
  ...
  draw() {
    if(this.hide) return;
    console.log('starLink.starLink.starLink',this.starLink);
    this.drawShape();
    this.isSelected && this.drawSelectedShape();
    (this.start?.isSelected || this.end?.isSelected) && this.drawSelectedShape();
  },
  isHere(cx, cy) {
  ...
  methods: {
    drawShape() {
      ...
      this.createLinePath();
      ctx.lineWidth = 1;
      ctx.strokeStyle = data.status === 'notExecute' ? '#C0C4CC' : data?.output?.color;
      if(this.isDelete){
        ctx.strokeStyle = 'red'
      }
      if(this.mouseDown && !( this.start.isSelected || this.end.isSelected)){
        ctx.globalAlpha = 0.6
        //ctx.strokeStyle= LightenDarkenColor( ctx.strokeStyle , -37)
      }else{
        ctx.globalAlpha = 1
      }
      ctx.stroke(); 
