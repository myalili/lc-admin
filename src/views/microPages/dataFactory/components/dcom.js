 ...
 */
 ...
 Uae.component('dcom', {
   ...
   draw() {
    ...
    ctx.fillStyle = '#749FD1'
    ctx.fillText(eval('("\\ue8bf")'), this.data.bounds.width*0.2, this.data.bounds.height/2);

    ctx.font = '15px arial'
    ctx.fillStyle = 'black';
    let text = this.data?.bindTable?.name || '输入'
    if(text.length > 5){
      text = text.substring(0,5)+'..'
    }
    ctx.fillText(text, parseInt(this.data.bounds.width/2),parseInt( this.data.bounds.height/2));
   },
   methods:{
