<template>
  <div class="schema-select-container">
    <div
      class="select-input flex flex-wrap"
      :class="{ 'py-2': schemsSeleted.length > 0 }"
    >
...
export default {
  ...
  computed: {
    isShow() {
      return (item) => {
        return (
          item.desc.includes(this.searchVal) ||
    ...
    operateChange(item) {
      if (item.checked == false) {
        item.children.forEach((element) => {
          ...
          if (
            this.schemsSeleted.findIndex((i) => i.name === element.name) > -1
          ) {
      ...
      this.schemasOptions.forEach((schema) => {
        if (schema.checked) {
          schema.children.forEach((schemaItem) => {
            if (
              schemaItem.checked &&
              !this.schemsSeleted.find((item) => item.id === schemaItem.id)
            ) {
