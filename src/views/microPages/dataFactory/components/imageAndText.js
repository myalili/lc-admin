  ...
  template: `<hider :hide="hide" :expand="expand" > </hider>`,
   ...
   methods: {
     drawShape() {
      ...
      // 舞台
      ctx.drawImage(stage, 10, 30, 60, 40)
      // 节点
      ctx.drawImage(node, 0, 0, 80, 80)


      // 标签
      ctx.font = `${textBox.fontSize} Arial`;
      let textWidth = ctx.measureText(data.nodeName).width
      let textHeight = parseInt(textBox.fontSize)

      let showText = data.nodeName
      
      
      const padding = 2 // 内边距
      const textBoxMaxWidth = bounds.width - 2
      const maxText = 5
      const lines = Math.ceil(data.nodeName.length/maxText)
      // 节点名称超边界处理
      // const overWidthFun = () => {
      //   if (textWidth > (textBoxMaxWidth - 2 * padding)) {
      //     // 计算超出字符
      //     const beyound = Math.ceil((textWidth - (textBoxMaxWidth - 2 * padding)) / parseInt(textBox.fontSize)) + 3
      //     showText = showText.slice(0, -beyound) + '...'
      //     textWidth = ctx.measureText(showText).width
      //     overWidthFun()
      //   }
      // }

      // overWidthFun()
      const fontSize = textBox.fontSize.substr(0,textBox.fontSize.length-2)
      const rectWidth =  fontSize * maxText + padding * 6 // 矩形尺寸
      const rectHeight=  textHeight + padding * 2 

      const rectPosition = {
        x: (bounds.width - rectWidth) / 2,
        y: bounds.height - rectHeight
      }
      ...
      ctx.beginPath()
      // 描边
      ctx.rect(rectPosition.x, rectPosition.y, rectWidth, rectHeight * lines);
      ctx.strokeStyle = '#8BCCF2';
      if (textBox.number) {
        ctx.strokeStyle =textBox.outline || 'rgba(255, 50, 50)';
      }
      ctx.lineWidth = 2;
      ctx.stroke();
      ctx.fillStyle = 'white';
      ...
      if (textBox.number) {
        ctx.fillStyle =textBox.outline || 'rgba(255, 50, 50)';
      }
      for(let i = 0 ; i < lines ; i++){
        ctx.fillText(data.nodeName.substring(i*maxText,i*(maxText)+maxText), textPosition.x, textPosition.y + i * rectHeight + padding);
      }
      //ctx.fillText(showText, textPosition.x, textPosition.y + padding);
      ctx.closePath()
      // 事件数
      if (textBox.number) {
        ctx.beginPath()
        ctx.arc(rectPosition.x + rectWidth, rectPosition.y, 8, 0, 2 * Math.PI)
        ctx.fillStyle =textBox.outline || 'rgba(255, 50, 50)';
        ctx.fill();
        ctx.closePath()
        ...
        ctx.textBaseline = 'middle';
        ctx.fillStyle = 'white';
        ctx.fillText(textBox.number, rectPosition.x + rectWidth,  rectPosition.y);
      }
