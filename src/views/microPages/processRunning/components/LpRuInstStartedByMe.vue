<template>
  <el-container class="lp-run-inst-started-by-me" direction="vertical" v-loading="listLoading">
    <el-header class="aops-search-box" height="auto" style="padding-top:0">
      <el-form :inline="true">
        <el-form-item label="流程实例ID">
          <el-input v-model="id" clearable placeholder="请输入" @change="handleSearch" />
        </el-form-item>
        ...
        <SpreadSearchCtn>
          ...
          <el-form-item label="发起时间">
            <AopsDatePicker v-model="timeRange" @change="handleSearch" />
          </el-form-item>
        </SpreadSearchCtn>
      </el-form>
    </el-header>
    <el-main style="padding: 0 20px;max-height:580px">
      <AopsTable :data="list" tooltip-effect="dark" style="width: 100%">
        ...
        <el-table-column min-width="80" prop="processDefinitionName" label="流程定义名称" show-overflow-tooltip />
        ...
        <el-table-column min-width="60" prop="startTime" label="发起时间" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.startTime | date }}</template>
        </el-table-column>
        <el-table-column min-width="100" prop="processDefinitionDescription" label="描述" show-overflow-tooltip />
...
<script>
const moduleName = 'afoeAdminLpRuInstStartedByMe';
