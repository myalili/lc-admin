...
const { queryLpRuInstStartedByMe } = window.getAopsApi();
// import { queryLpInstStartedByMe } from '@/api/workflowApi';
...
export default {
  ...
  getters: {
    getField,
    startedAfter(state) {
      return state.query.timeRange?.[0];
    },
    startedBefore(state) {
      return state.query.timeRange?.[1];
    }
  },
  mutations: {
    updateField,
  ...
  actions: {
    ...
    async load({
      ...
      try {
        const res = await getInvokeResponse(queryLpRuInstStartedByMe(queryParams), '查询我发起的流程实例');
