  ...
  <div class="string_input">
    ...
    <div class="">
      <div
        ...
        v-for="(itemi, indexi) in obj.model[item.name]"
        :key="itemi.wid"
      ...
      >
        <div class="title relative">
          ...
          <span :title="itemi.title" v-if="!itemi.edit">{{ itemi.title }}</span>
          <el-input
            size="mini"
            style="width: 150px"
            v-model="itemi.title"
        ...
        <div class="content">
          ...
          <div class="proto">
            ...
            <el-input
              ...
              style="width: 150px"
              v-model="itemi.path"
              v-if="(itemi.type === 'WINDOW' && itemi.edit) || (itemi.type === 'js' && itemi.edit)"
            ></el-input>
            <span
              v-if="(itemi.type === 'WINDOW' && !itemi.edit) || (itemi.type === 'js' && !itemi.edit)"
              :title="itemi.edit"
...
export default {
  mixins: [ require,  window.codelessMixins.pageList ],
  props: {
    obj: {
      type: Object,
      default: () => {
  ...
  created() {
    ...
    const ctx = this;
    ctx.obj.model[ctx.item.name].forEach((element) => {
      if (!element.hasOwnProperty("edit")) {
        ctx.$set(element, "edit", true);
        ctx.$set(element, "hoverIndex", -1);
      }
    });
  },
  computed: {},
  methods: {
    add() {
      const ctx = this;
      console.log(ctx.obj.model[ctx.item.name]);
      console.log(ctx.item.name);
      ctx.obj.model[ctx.item.name].push({
        title: "新页面" + ctx.obj.model[ctx.item.name].length,
        type: "",
        path: "",
        hoverIndex: -1,
        edit: true,
        ...
        wid: ctx.obj.model.wid + '-' + ctx.obj.model[ctx.item.name].length
    ...
    del(itemi, indexi) {
      ...
      if (itemi.path?.prototypeId) {
          ctx.delPageInPageContent(itemi.wid);
        }
    },
    // choosePage(e, itemi) {
