<template>
  <div class="table-container flex flex-col justify-between">
    ...
    >
      <el-table-column type="selection" width="55" v-if="!dataTable.notMultipleTable && !entityId" />
        ...
        <template slot-scope="scope">
          ...
          <template v-else>
            <template v-if="item.format && item.format === 'uploadImg'">
              <el-image style="width: 100px; height: 100px;" :src="dataIndexFilter(scope.row[item.dataIndex], item)" fit="cover"></el-image>
            </template>
            <template v-else-if="item.format && item.format === 'uploadDocs'">
              <el-link type="primary" @click.native="openDocsLink(scope.row[item.dataIndex])">{{ dataIndexFilter(scope.row[item.dataIndex], item) }}</el-link>
            </template>
            <component
        ...
        <template slot-scope="scope">
          <template v-for="item in dataTable.tableOpr">
            ...
            <template v-else-if="item.opr.id === 'popconfirm'">
              <el-popconfirm icon="el-icon-warning" :title="item.popconfirm.title" :key="item.value">
...
export default {
  ...
  mounted() {
    console.log('有没有渲染',this.dataTable);
  },
  watch: {
    ...
    noEnumCodesObj: {
      handler(n, o) {
        ...
        if (JSON.stringify(n) !== '{}') {
          let trueArr = []
          for (const key in n) {
            if (n[key]) trueArr.push(n[key])
          }
          if (trueArr.filter(i=> i === false).length === Object.keys(ctx.enumCodesObj).length) {
            ctx.getPageList();
          }
        }
      },
  ...
  computed: {
    ...
    columns() {
      ...
      if (ctx.dataTable.entityId) {
        columns = ctx.dataTable.tablePro.map((i) => {
          let key = i.key || i.dictionaryItem.engNameShort;
          // i.code && ctx.searchEnums(i.code, i.format);
          if (i.format) {
            ctx.searchEnums(i.code, i.format);
          }
          return {
            ...i,
    ...
    dataIndexFilter() {
      ...
      return (value, item) => {
        ...
        if (item.format === 'dict') {
          if (ctx.enumCodesObj[item.code]) {
            return activator.get('alcap.tableDataFilter')[item.format].handler(value, item, ctx)
          }else {
            ctx.$set(ctx.noEnumCodesObj, item.code, true);
            return value
          }
        }else {
          return activator.get('alcap.tableDataFilter')[item.format].handler(value, item, ctx)
  ...
  methods: {
    ...
    async searchEnums(enumCode, format) {
      if (activator.get('alcap.tableDataFilter')[format].beforeHandler) {
        this.enumCodesObj[enumCode] = await activator.get('alcap.tableDataFilter')[format].beforeHandler(enumCode);
        this.$set(this.noEnumCodesObj, enumCode, false);
      }else {
        return
      }
    },
    ...
    async getPageList(page, size) {
      ...
      if(ctx.newEntity.type === 'data_source') {

        let arr = [ { value: ctx.currentPage, name: 'pageNum'}, { value: ctx.pageSize, name: 'pageSize'} ]
        arr.forEach(key => {
          let entityFieldId = ctx.newEntity.fields.find(i=>i.dictionaryItem.engNameShort === ctx.dataTable[key.name]).id
          let params = {
            entityFieldId,
            value: key.value
          }
          let index = ctx.dataSourceQuery.findIndex(i=>i.entityFieldId === entityFieldId)
          if(index > -1) {
            ctx.dataSourceQuery[index].value = params.value
          }else {
            ctx.dataSourceQuery.push(params)
          }      
        });     
      }
      ...
      if (!ctx.entityUrl.url) {
        pipe.entity
          ...
          .then((res) => {
            if (res) {
              ...
              let tableData = (res.response && res.response.content) || [res.response];
              ...
              if (ctx.dataTable.display === "tree") {
                ctx.dataSource = ctx.transformMenu(ctx.dataSource, ctx.dataTable.parentField || 'PARENT_ID');
              }
              ctx.totalCount =
                (res.response && res.response.totalElements) || 0;
                
              // if(res.entity.type === 'data_source') ctx.totalCount = res.response[ctx.dataTable.totalElements] || 0;
              ...
              if (ctx.entityId) {
                ctx.baseColumns = Object.keys(ctx.dataSource[0]).map((i) => {
                  i === "title" ? "dataTitle" : i;
                  return {
                    title: res.entity.type === 'db' ? res.entity.fields.find((e) => e.name === i)?.desc : i,
                    dataIndex: i,
                    align: "center",
        ...
        let obj = await ctx.entityUrl.url(paramsMap.get(ctx.entityUrl.type));
        ctx.loading = false;

        if(ctx.entityUrl.type === 'data_source') obj = JSON.parse(obj);
        let tableData = (obj && obj.content) || [obj];
        ctx.dataSource = tableData.map(function (item) {
          return {
            ...item,
          };  
        ...
        console.log('ctx.currentPage', ctx.currentPage, ctx.totalCount);

        // if(ctx.newEntity.type === 'data_source') ctx.totalCount = obj[ctx.dataTable.totalElements] || 0;
      }
    },
