...
export default {
  ...
  methods: {
    ...
    submitForm() {
      ...
      ctx.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          ctx.$emit("submit", ctx.searchForm);
        } else {
    ...
    async init() {
      ...
      let entity = await pipe.entity.getEntityDetail(
        ctx.dialogInfo.opr.entityId,
        false
      );
      ...
      ctx.dialogInfo.fields.map((i) => {
        ...
        if (ctx.dialogInfo.opr.opr.operationType === "create") {
          ctx.$set(ctx.searchForm, i.dictionaryItem.engNameShort, "");
        }
      });

      console.log("数据：", ctx.dialogInfo.opr.entityId.fields);
