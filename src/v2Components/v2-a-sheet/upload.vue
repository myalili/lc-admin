<template>
  <div class="flex items-center">
    ...
    >
      <el-button style="margin-right:10px" type="" size="mini">导入表格</el-button>
    </el-upload>
  </div>
</template>
...
export default {
  ...
  methods: {
    async customUpload() {
      ...
      // });
      let res = await pipe.entity.getEntityDetail(ctx.entityId);
      ...
      urlMap[res.type]({ entityId: res.id, file }).then((res) => {
        if (res.success) {
          ...
          ctx.fileList = []
          ctx.$emit('handleChange', ctx.entityId);
        } else {
